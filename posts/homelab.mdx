...
The network currently is currently seggregated into 4 VLANS: public wifi, private wifi, always VPN'd devices, and the general all else internal net. The VPN'd VLAN is setup as an interface for a Wiregaurd tunnel, which is used to route all traffic for that interface (it has firewall rules to prevent any WAN tunneled packets). I also have an OpenVPN server setup because a [specific type of network traffic](https://support.torproject.org/abuse/what-about-ddos/#:~:text=But%20because%20Tor%20only%20transports%20correctly%20formed%20TCP%20streams%2C%20not%20all%20IP%20packets%2C%20you%20cannot%20send%20UDP%20packets%20over%20Tor.) only allows TCP streams which [Wiregaurd doesn't support](https://www.wireguard.com/known-limitations/#:~:text=WireGuard%20explicitly%20does%20not%20support%20tunneling%20over%20TCP).

I have a Raspberry Pi 4 running PowerDNS, which is used as the primary DNS server for the network and it's configured to use [NextDNS](https://nextdns.io/) for the upstream provider. I also have a local TailScale node running on the Pi 4 and a remote node in my parents house. I use the remote node to watch Netflix (to mask as same household devices). (I'd like to automatically route Netflix traffic through the remote node but have run into MTU issues with the DNS. For now, I manually route the traffic with firewall rules.)
...
> For learning purposes I sometimes configure another VLAN which routes to a VM with a Wireguard server and mitmproxy. You'd have to setup root SSL certs to intercept anything but nevertheless I keep this as private network.
...
- NVIDIA RTX 3050 (Patched for transcoding)
- 1 [PNY 250Gb SSD](https://amzn.to/3nfbhBp) (OS)
***The drives below are SMR drives, I don't reccommend***
- 4x [Seagate BarraCuda 4Tb](https://amzn.to/41SfKsP) Drive (Pool 1 - RAIDZ1)
- 2x 2Tb random Drives (Pool 2 - RAIDZ1)

Zeus is the granddaddy of all my servers. He's been through more disasters than a Greek tragedy: caseless for months, AIO leaks, you name it. But like a fine wine, he only gets better with age. Zeus is where most of the action happens, with a plethora of virtual machines running amok. The main players include:

- Storage Server | TrueNas Scale
- Apollo | Debian w/ Portainer (QBt, the *arr’s, etc)
- Artemis | Debian w/ GitLab
...
Poseidon may be the God of the Sea but also doubles as the backup server for the Lab. No longer in a cluster, he hosts some backup VMs and mirrors, along with some utilities like an SMTP server and NGINX proxy and some testing environments.

## Hades: The Gaming Servers

*THE GOD OF THE UNDERWORLD, and mischief...*

Hades is the ultimate wingman for your gaming needs. Cost, deployment, and management are all taken care of by Hades right from the UNDERWORLD... I mean, the cloud.

Hades uesd to be couple of inordinately large python scripts run in a created Minecraft, Rust, and Valheim game servers by gaming the cloud 'new-account-credit' system. It used Selenium, Privacy.com and Paramiko to create new cloud accounts with free credits, deploy a server, and then manage the server. With the intent of creating a free game server everytime. But it was a mess, rarley worked, and most cloud providers have since stopped accepting such card provide. I've since moved to using [Pterodactyl](https://pterodactyl.io/) to manage the game servers.
...
> Apollo is also no landlubber — he frequents the high seas with ease, thanks to his mighty crew. Among his crew, he counts quartermaster Portainer, qBittorrent, Sonarr, Radarr, Overseerr, Prowler, Jacket, and flare resolver, all of whom are ready to plunder the vast seas of the internet for treasure. But this pirate's adventure never ends, and he's even considering some Usenet dabbling.
...
Artemis gives life to my code. She is a collection of tools I use to deploy my applications. I host my own GitLab instance here, which is used to host personal projects and to host some CI/CD pipelines. She's hosted on Zeus and is part of the crucial backup media. I also host some non-GitLab CD pipelines, a motley crew of bash scripts and Ansible playbooks that are triggered by various hooks.
